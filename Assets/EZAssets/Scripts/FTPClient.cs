...
//    }
    ...
    public FTPClient(string host, string userId, string password, int port, IWebProxy proxy, bool enableSsl, bool useBinary, bool usePassive)
    {
        ...
        {
            this.host = "ftp://" + host + ":" + port.ToString();
        }
        this.port = port;
        this.proxy = proxy;
	...
	public bool Upload(string path, string remoteFileName)
	{
		FileInfo info = new FileInfo (path);
		return Upload (info, remoteFileName);
	}
    ...
    public Stream DownloadXMl(string serverName)
    {
        string url = Host.TrimEnd('/') + RemotePath + serverName;
        FtpWebRequest request = CreateRequest(url, WebRequestMethods.Ftp.DownloadFile);
    ...
    public string DownloadFileStringContent(string serverName)
    {
        string url = Host.TrimEnd('/') + RemotePath + serverName;
        FtpWebRequest request = CreateRequest(url, WebRequestMethods.Ftp.DownloadFile);
        FtpWebResponse response = (FtpWebResponse)request.GetResponse();
        ...
        if (resStream != null)
        {
            ...
            int readSize = 0;
            readSize = resStream.Read(buffer, 0, 1024);
            while (readSize > 0)
            {
            ...
            resStream.Close();
            string contentStr = System.Text.Encoding.UTF8.GetString(byteList.ToArray());
            return contentStr;
        }
        return null;
    }
    ...
    public List<string> GetFileList(string dicName)
    {
        List<string> result = new List<string>();
        string url = Host.TrimEnd('/') + RemotePath + dicName;
        FtpWebRequest request = CreateRequest(url, WebRequestMethods.Ftp.ListDirectory);
    ...
    #region 上传一个文件夹到远端路径下
    public bool Upload(DirectoryInfo dInfo, string startPath)
    {
        if (dInfo.Exists)
        {
            string dirPath = dInfo.FullName.Substring(dInfo.FullName.LastIndexOf(startPath));
            MakeDirectory(dirPath);
            FileInfo[] files = dInfo.GetFiles();
            foreach (FileInfo fi in files)
            {
                Upload(fi, fi.FullName.Substring(fi.FullName.LastIndexOf(startPath)));
            }
        ...
        throw new Exception(string.Format("本地文件不存在,文件路径:{0}", dInfo.FullName));
    }
    #endregion
